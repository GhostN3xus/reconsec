...
func main() {
	...
	case "activescan":
		...
		for i, a := range os.Args {
			if a == "-url" && i+1 < len(os.Args) {
				urlArg = os.Args[i+1]
			}
			if a == "-payloads" && i+1 < len(os.Args) {
				payloadsPath = os.Args[i+1]
			}
			if a == "--sandbox" && i+1 < len(os.Args) && os.Args[i+1] == "true" {
				sandbox = true
			}
			if a == "--confirm-authorized" && i+1 < len(os.Args) {
				confirm = os.Args[i+1]
			}
		}
	...
	case "test":
		...
		if len(os.Args) < 3 {
			fmt.Println("usage: reconsec test <url>")
			return
		}
		url := os.Args[2]
